...
export class RyuClient {
	...
	/** Agent inboxes, messages, threads, webhooks, and delivery events. */
	readonly mail: MailAPI;
	/** Optional standalone Notify API, configured with its own tenant key. */
	readonly notify: NotifyAPI | null;
	...
	constructor(options: RyuClientOptions) {
		...
		this.mail = new MailAPI(options);
		this.notify = options.notify ? new NotifyAPI(options.notify) : null;
	}
}
