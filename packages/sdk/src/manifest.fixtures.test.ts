...
function changedSourceFields(
	...
	if (Array.isArray(source)) {
		...
		for (const [index, value] of source.entries()) {
			changedSourceFields(value, parsed[index], `${path}[${index}]`, changed);
		}
...
describe("SDK preserves every authored field in supported manifests", () => {
	for (const { file, manifest } of FIXTURES) {
		const parsed = PluginManifestSchema.safeParse(manifest);
		if (parsed.success) {
			it(file, () =>
				expect(changedSourceFields(manifest, parsed.data)).toEqual([])
			);
		}
	}
});
