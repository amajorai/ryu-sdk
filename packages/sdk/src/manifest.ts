...
/** How Core composes a contributed turn hook. */
export const HookModeSchema = z.enum(["directive", "middleware"]);
...
export const TurnHookContributionSchema = z
	.object({
		...
		/**
		 * Composition mode. Omitted keeps the independent directive behavior;
		 * `middleware` gives the function a `next(ctx)` continuation.
		 */
		mode: HookModeSchema.optional(),
		...
		/** Core orders hooks by signed 32-bit priority. */
		priority: z
			.number()
			.int()
			.min(-2_147_483_648)
...
export const ContributesSchema = z
	.object({
		/** Core and the host own these declarative contribution payloads. */
		context_menu_items: z.array(z.record(z.string(), z.unknown())).default([]),
		views: z.array(z.record(z.string(), z.unknown())).default([]),
		store_tabs: z.array(z.record(z.string(), z.unknown())).default([]),
		create_actions: z.array(z.record(z.string(), z.unknown())).default([]),
		agent_edit_panels: z.array(z.record(z.string(), z.unknown())).default([]),
