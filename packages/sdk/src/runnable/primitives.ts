...
import type { BackupOperation, BackupRecord } from "../backups.types.ts";
 ...
 */
/** Encrypted app snapshots. Requires `backups:app` and explicit destination access.
 * Core derives app and tenant scope; credentials never enter the app. */
export interface BackupsClient {
	...
	destinations(): Promise<Array<{ id: string; name: string }>>;
	get(input: {
		destinationId: string;
		namespace?: string;
		operationId: string;
...
export function createPrimitives(transport: PrimitiveTransport): RyuPrimitives {
	return {
		backups: {
			destinations: () =>
				transport.bridge("backups.destinations", {}) as ReturnType<
					BackupsClient["destinations"]
				>,
			create: (input) =>
				transport.bridge("backups.create", input) as Promise<BackupOperation>,
			list: (input) =>
				transport.bridge("backups.list", input) as Promise<BackupRecord[]>,
			get: (input) =>
				transport.bridge("backups.get", input) as Promise<BackupOperation>,
			restore: (input) => transport.bridge("backups.restore", input),
		},
		background: {
			list: (input) =>
