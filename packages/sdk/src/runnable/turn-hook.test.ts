...
describe("defineTurnHook", () => {
	...
	it("serializes a middleware hook with a typed continuation", () => {
		const hook = defineFunctionHook({
			id: "x.middleware",
			on: "tool_result",
			priority: 8,
			match: { tools: ["bash*"] },
			run: async (ctx, _host, next) => {
				const result = await next({ ...ctx, event: { wrapped: true } });
				return result.kind === "none"
					? { kind: "note", text: "no downstream result" }
					: result;
			},
		});
		expect(hook.mode).toBe("middleware");
		expect(hook.code).toContain("(ctx, host, next)");
		expect(hook.code).toContain("next({ ...ctx");
		expect(hook.priority).toBe(8);
		expect(hook.match).toEqual({
			commands: [],
			stateful: false,
			tools: ["bash*"],
		});
